...
namespace llmcpp
{
  ...
  class gpt2_tokenizer {
    ...
    //const static inline std::string regex_pattern = "'s|'t|'re|'ve|'m|'ll|'d| ?\\p{L}+| ?\\p{N}+| ?[^\\s\\p{L}\\p{N}]+|\\s+(?!\\S)|\\s+";
    //const static inline std::string regex_pattern = R"('s|'t|'re|'ve|'m|'ll|'d| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+)";
  ...
  bool gpt2_tokenizer::load(std::string_view vocab_file, std::string_view merges_file)
  {
    // load merges file
